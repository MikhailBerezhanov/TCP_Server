...
BIN_DIR = ./bin
BIN_NAME = tcp_serv.out
OBJ_DIR = ./obj
TESTS_DIR=./tests

OBJS = $(addprefix $(OBJ_DIR)/, sequence.o server.o main.o)
...
mem_check: CXXFLAGS = -std=c++17 -g -O0

mem_check: prep bin
	@valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --log-file=$(OUTPUT_FILE) $(BIN_DIR)/$(BIN_NAME)
	@cat $(OUTPUT_FILE)